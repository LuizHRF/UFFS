...
modelo_ia_2 = HDBSCAN(min_samples=2, store_centers="centroid")
# Treina o modelo
modelo_ia_1.fit(x)
modelo_ia_2.fit(x)
...
# Ordena dados usando coluna de agrupamento
data = data.sort_values(by=['Grupo HDBSCAN'])
# Salva os resultados em arquivo
print("\nResultado do HDBSCAN")
data.to_csv('resultado_2.csv', index=False)
with pd.option_context('display.max_rows', None, 'display.max_columns', 4):
    print(data)
...
# Crie um DataFrame com os centros dos grupos
centros = pd.DataFrame(modelo_ia_2.centroids_, columns=data.columns.values[1:-2])
# Arredonda valores com 2 casas decimais
centros = centros.round(2)
# Adicione uma coluna para identificar os centros dos grupos
centros['Nome'] = ['Centro ' + str(i) for i in range(len(modelo_ia_2.centroids_))]
# Salva os resultados em arquivo
print("\nCentros do HDBSCAN")
